            ...
            if (toEdit != null)
            {
                ...
                foreach (Transform child in toEdit.transform)
                {
                    if (GUILayout.Button(child.name))
                        childToEdit = child.gameObject;
                }
                GUILayout.Label("");
                if (childToEdit != null)
                {
                    GUILayout.Label("Selected child's " + "(" + childToEdit.name + ")" + " Components:");
                    GUILayout.Label("Note: This child gameObject is attached to " + childToEdit.transform.parent.name + " (This is the parent).");
                    foreach (Component c in childToEdit.GetComponents(typeof(Component)))
                    {
                        GUILayout.Button(c.GetType().Name);
                    }
                }
